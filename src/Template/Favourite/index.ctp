<div class="favourite index">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<tbody>
			<?php foreach ($favourite as $i => $favourite): ?>
				<tr>
					<td>
						<?php
						echo $this->AlaxosForm->checkBox('Favourite.' . $i . '.id', array('value' => $favourite->id, 'class' => 'model_id'));
						?>
					</td>
					<td>
					...
					<td>
						<?php echo h($favourite->to_display_timezone('created')); ?>
					</td>
					<td>
						<?php echo h($favourite->to_display_timezone('modified')); ?>
					</td>
					<td class="actions">
						<?php 
// 						echo $this->Navbars->actionButtons(['buttons_group' => 'list_item', 'buttons_list_item' => [['view', 'edit', 'delete']], 'model_id' => $favourite->id]);
						?>
						
						<?php 
// 						echo $this->Html->link('<span class="glyphicon glyphicon-search"></span> ' . __d('alaxos', 'view'), ['action' => 'view', $favourite->id], ['class' => 'to_view', 'escape' => false]);
// 						echo ' ';
// 						echo $this->Html->link('<span class="glyphicon glyphicon-pencil"></span> ' . __d('alaxos', 'edit'), ['action' => 'edit', $favourite->id], ['escape' => false]);
// 						echo ' ';
// 						echo $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span> ' . __d('alaxos', 'delete'), ['action' => 'delete', $favourite->id], ['confirm' => __('Are you sure you want to delete # {0}?', $favourite->id), 'escape' => false]);
						?>
						
						<?php 
						echo $this->Html->link('<span class="glyphicon glyphicon-search"></span>', ['action' => 'view', $favourite->id], ['class' => 'to_view', 'escape' => false]);
						echo '&nbsp;&nbsp;';
						echo $this->Html->link('<span class="glyphicon glyphicon-pencil"></span>', ['action' => 'edit', $favourite->id], ['escape' => false]);
						echo '&nbsp;&nbsp;';
						echo $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span>', ['action' => 'delete', $favourite->id], ['confirm' => __('Are you sure you want to delete # {0}?', $favourite->id), 'escape' => false]);
						?>
					</td>
				</tr>
			<?php endforeach; ?>
			...
			<?php
			if(isset($favourite) && $favourite->count() > 0)
			{
				echo '<div class="row">';
