<div class="body index">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			<tbody>
			<?php foreach ($body as $i => $body): ?>
				<tr>
					<td>
						<?php
						echo $this->AlaxosForm->checkBox('Body.' . $i . '.id', array('value' => $body->id, 'class' => 'model_id'));
						?>
					</td>
					<td>
						<?php echo h($body->background) ?>
					</td>
					<td>
						<?php echo h($body->first_label) ?>
					</td>
					<td>
						<?php echo h($body->second_label) ?>
					</td>
					<td>
						<?php echo h($body->third_label) ?>
					</td>
					<td>
						<?php echo $body->has('template') ? $this->Html->link($body->template->id, ['controller' => 'Templates', 'action' => 'view', $body->template->id]) : ''; ?>
					</td>
					<td>
						<?php echo h($body->to_display_timezone('created')); ?>
					</td>
					<td>
						<?php echo h($body->to_display_timezone('modified')); ?>
					</td>
					<td class="actions">
						<?php 
// 						echo $this->Navbars->actionButtons(['buttons_group' => 'list_item', 'buttons_list_item' => [['view', 'edit', 'delete']], 'model_id' => $body->id]);
						?>
						
						<?php 
// 						echo $this->Html->link('<span class="glyphicon glyphicon-search"></span> ' . __d('alaxos', 'view'), ['action' => 'view', $body->id], ['class' => 'to_view', 'escape' => false]);
// 						echo ' ';
// 						echo $this->Html->link('<span class="glyphicon glyphicon-pencil"></span> ' . __d('alaxos', 'edit'), ['action' => 'edit', $body->id], ['escape' => false]);
// 						echo ' ';
// 						echo $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span> ' . __d('alaxos', 'delete'), ['action' => 'delete', $body->id], ['confirm' => __('Are you sure you want to delete # {0}?', $body->id), 'escape' => false]);
						?>
						
						<?php 
						echo $this->Html->link('<span class="glyphicon glyphicon-search"></span>', ['action' => 'view', $body->id], ['class' => 'to_view', 'escape' => false]);
						echo '&nbsp;&nbsp;';
						echo $this->Html->link('<span class="glyphicon glyphicon-pencil"></span>', ['action' => 'edit', $body->id], ['escape' => false]);
						echo '&nbsp;&nbsp;';
						echo $this->Form->postLink('<span class="glyphicon glyphicon-trash"></span>', ['action' => 'delete', $body->id], ['confirm' => __('Are you sure you want to delete # {0}?', $body->id), 'escape' => false]);
						?>
					</td>
				</tr>
			<?php endforeach; ?>
